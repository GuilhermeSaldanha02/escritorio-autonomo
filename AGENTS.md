...
- **Em andamento:** M2, M3 e **M4 completos, aprovados pela revisão externa e integrados em `staging`/`main`** (tags `m2-primeiro-ciclo`, `m3-inteligencia-governada`, `m4-cacador-real`). **M5 (Memória + Economia): plano escrito (`docs/M5-PLANO.md`), consultado com a revisão externa, 20 critérios de aceite definidos. Nenhuma linha de código ainda — aguardando revisão do documento e autorização explícita do dono para começar a implementação.**
- **Não commitado:** nada.
- **Bloqueado / a decidir:** implementação do M5 aguarda autorização do dono (mesmo processo do M2/M3/M4 — plano primeiro, código depois).
- **Para o outro agente saber:** decisões estruturais do plano do M5 (`docs/M5-PLANO.md`), fechadas com a revisão externa antes de qualquer código: `PAID` é alcançável no M5, mas só via `PaymentEvidence` explicitamente `SIMULATED`/`TEST` — nunca conta como receita externa real nem marca `SELF_SUSTAINING` (isso é M8). Memória nunca vem de IA: `ExperienceBuilder` deriva `Experience` só de evidência já persistida (task/attempts/review/events/model_calls/tool_calls); `Experience` só vira `Memory` corporativa via `MemoryProposal → MemoryValidator`, nunca automaticamente. Performance de agente é calculada no M5, mas transição de lifecycle (`PROBATION → ACTIVE` etc.) é M6, não M5. Dinheiro sempre em centavos inteiros, nunca float; split 50/30/20 precisa conservar exatamente o valor total. Reconciliação detecta inconsistência mas não conserta sozinha no M5.
