...
- **Em andamento:** M2 completo, aprovado e integrado em `staging`/`main` (tag `m2-primeiro-ciclo`); repositório publicado no GitHub (`GuilhermeSaldanha02/escritorio-autonomo`, privado). **M3 (Inteligência Governada) implementado, validado e aprovado pela revisão externa** ("M3 — APROVADO PARA MERGE ✅", ChatGPT, 2026-09-17) — os 7 passos e os 16 critérios de aceite têm prova reexecutável em `docs/M3-PLANO.md`/`docs/M3-INTELIGENCIA-GOVERNADA.md`. Quatro pacotes novos: `@escritorio/budget` (reserva atômica de orçamento), `@escritorio/ai` (AI Gateway + Model Router, `AI_MODE=mock` operacional, `local`/`api` `NOT_CONFIGURED` de propósito), `@escritorio/tool-gateway` (Tool Gateway, `GovernedSandbox`). Desenvolvedor/Revisor/Diretor do M2 foram religados para passar pelos Gateways — toda a suíte do M2 continua passando sem alteração.
- **Não commitado:** nada
- **Bloqueado / a decidir:** nada — falta só a autorização explícita do dono para o merge de `feat/m3-inteligencia` em `staging`/`main`.
- **Próximo passo:** aguardar autorização do dono para o merge do M3. Depois disso, **antes de começar o M4 (Caçador Real), desenhar os critérios de aceite com a revisão externa primeiro** — a própria revisão apontou que M4 (fontes públicas reais, deduplicação, verificação de recompensa, potencial conteúdo malicioso) é "uma fronteira de segurança bem diferente de M1–M3" e merece o mesmo processo de design prévio já usado antes do M2 e do M3, não implementação direta.
- **Para o outro agente saber:** decisão explícita, respeitada em todo o M3: **nenhum LLM local nem API paga real** — `local`/`api` são adapters reais testados contra servidor fake em localhost, mas ficam `NOT_CONFIGURED` em operação. Limitação conhecida e registrada (não crítica, revisada e aceita pela revisão externa): a chamada do Diretor ao AI Gateway (`opportunity-handler.ts`) não é atômica com a transição de estado — uma reentrega tardia do BullMQ pode duplicar a linha de auditoria em `model_calls` (não o gasto, que é protegido por `idempotencyKey` na reserva de orçamento). Antes de ativar qualquer provider pago real, a revisão pediu para resolver isso com um `logical_ai_call_id` (`UNIQUE`) — idempotência da própria chamada, não uma transação de banco aberta durante I/O externo. `packages/agents/src/developer.ts`/`reviewer.ts` agora aceitam `Pick<SandboxManager, 'run'>` em vez da classe concreta — é só o tipo, a lógica é a mesma; isso foi necessário porque `SandboxManager` tem campos privados e não aceita duck typing estrutural. Confira RAM livre antes de testes de integração pesados (a máquina já chegou a ~245MB livres no M2; ficou entre 400MB-1.2GB durante o M3, sem incidente).
