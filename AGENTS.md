...
- **Última sessão:** 2026-09-18 (madrugada) · agente: claude · branch: `feat/m5-memoria-economia` (de `staging`), sozinho enquanto o dono dormia (autorizado a continuar e a consultar a revisão externa em pontos de decisão, sem mergear sem autorização) — dono acordou, autorizou o merge do M4 ("então faça") e depois "pode seguir" para reabrir a especificação e planejar o M5.
- **Em andamento:** M2, M3 e **M4 completos, aprovados pela revisão externa e integrados em `staging`/`main`** (tags `m2-primeiro-ciclo`, `m3-inteligencia-governada`, `m4-cacador-real`). **M5 (Memória + Economia): plano escrito e APROVADO PARA IMPLEMENTAÇÃO pela revisão externa** (`docs/M5-PLANO.md`, 20 critérios de aceite, 2 rodadas de revisão — a segunda incorporou 3 ajustes: política de arredondamento normativa, `ledger_scope` SIMULATION/REAL isolado, embedding determinístico marcado como não-semântico). **Nenhuma linha de código ainda — aguardando só a autorização explícita do dono para começar a implementação.**
- **Não commitado:** nada.
- **Bloqueado / a decidir:** implementação do M5 aguarda autorização do dono — o plano já está aprovado tecnicamente, só falta o "pode começar" (mesmo processo do M2/M3/M4).
- **Para o outro agente saber:** decisões estruturais do plano do M5 (`docs/M5-PLANO.md`), fechadas com a revisão externa antes de qualquer código: `PAID` é alcançável no M5, mas só via `PaymentEvidence` explicitamente `SIMULATED`/`TEST` — todo lançamento de `financial_ledger` carrega `ledger_scope` (`SIMULATION`/`REAL`), e `SIMULATION` nunca soma em caixa/receita real nem em `SELF_SUSTAINING` (isso é M8). Memória nunca vem de IA: `ExperienceBuilder` deriva `Experience` só de evidência já persistida; `Experience` só vira `Memory` corporativa via `MemoryProposal → MemoryValidator`, nunca automaticamente. `DeterministicEmbeddingProvider` prova infraestrutura vetorial (pgvector, top-K, filtro de escopo), nunca qualidade semântica — não descrever como "memória semântica" no relatório de fechamento. Performance de agente é calculada no M5, mas transição de lifecycle (`PROBATION → ACTIVE` etc.) é M6, não M5. Split 50/30/20 usa `floor` por bucket com resíduo sempre para `RESERVE` (regra normativa, não escolha livre da implementação). Reconciliação detecta inconsistência mas não conserta sozinha no M5.
