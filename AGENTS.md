...
- **Em andamento:** M2 completo — todos os 6 passos e os 11 critérios de aceite têm prova (✅) em `docs/M2-PLANO.md`. M1 aprovado e integrado em `staging` e `main` (tag `m1-fundacao`). M2 ainda não integrado em `staging`/`main` — falta relatório final + ZIP + revisão externa + autorização do dono antes disso e antes do M3 (mesma disciplina do M1).
- **Não commitado:** nada
- **Bloqueado / a decidir:** nada pendente de infraestrutura. Validação completa (typecheck, lint, 89 unitários, 57 integração, build limpo de `dist/` zerado) passa de estado limpo. Limitações de ambiente do passo 4 (`pidsLimit` baixo trava; teste de rede por hostname é flaky) seguem documentadas em `docs/M2-PLANO.md`. Um gap latente do critério 9 (task presa em `ASSIGNED` sem evento se `TASK_START` negar e os retries do job esgotarem) também está documentado lá — inofensivo com um só Worker.
- **Próximo passo:** fechar o M2 — relatório com os 20 itens no padrão do M1 (`docs/M1-FUNDACAO.md`) + ZIP (`git archive`, sem `.env`/`node_modules`/`dist`) + revisão externa (mesmo chat do ChatGPT usado no M1 e na revisão intermediária do passo 5) + autorização explícita do dono antes de tocar no M3.
